...
const int_range_t default_neuron_limits[] = {{32, 128}, {16, 64}, {8, 32}};
const int default_batch_choices[] = {32, 64, 128};
const layer_pattern_t default_layer_patterns[] = {HALVE, SAME};
const int default_[] = {HALVE, SAME};
...
template <typename T> T random_choice(const T *choices, long num_choices) {
...
layer_pattern_t random_layer_pattern() {
  return random_choice(PATTERNS.original.data(), PATTERNS.original.size());
}
...
double random_decay() {
  return random_between(limits.decay.min, limits.decay.max);
}
optimizer_t random_optimizer() {
  return random_choice(OPTIMIZERS.original.data(), OPTIMIZERS.original.size());
}
